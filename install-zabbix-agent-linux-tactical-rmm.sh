...
#   $7 = {{agent.ZabbixHostName}}       Custom Zabbix hostname (optional, defaults to system hostname)
#   $8 = "force"                         Force re-run even if already on target version (optional)
# =============================================================================
...
ZABBIX_HOSTNAME=$(echo "${7:-}"  | tr -d "'")
FORCE_RUN=$(echo "${8:-}"        | tr -d "'" | tr '[:upper:]' '[:lower:]')
...
AGENT_HOSTNAME="${ZABBIX_HOSTNAME:-$SYS_HOSTNAME}"
OS_ID=$(. /etc/os-release && echo "$ID")
OS_CODENAME=$(. /etc/os-release && echo "$VERSION_CODENAME")
OS_VERSION=$(. /etc/os-release && echo "$VERSION_ID")

log "Host:    $SYS_HOSTNAME ($IP_ADDRESS)"
[[ "$AGENT_HOSTNAME" != "$SYS_HOSTNAME" ]] && log "Zabbix Hostname: $AGENT_HOSTNAME"
...
if service_active mssql-server || systemctl list-units --type=service 2>/dev/null | grep -q mssql; then
    ...
    if ! package_installed zabbix-agent2-plugin-mssql; then
        ...
        DEBIAN_FRONTEND=noninteractive apt-get install -y -qq -o Dpkg::Options::="--force-confold" zabbix-agent2-plugin-mssql
    else
...
if service_active redis-server || service_active redis; then
    ...
    # Install the Redis loadable plugin if not already installed
    if ! package_installed zabbix-agent2-plugin-redis; then
        log "  Installing zabbix-agent2-plugin-redis..."
        DEBIAN_FRONTEND=noninteractive apt-get install -y -qq -o Dpkg::Options::="--force-confold" zabbix-agent2-plugin-redis 2>/dev/null || true
    else
        log "  zabbix-agent2-plugin-redis already installed"
    fi

    # Write session config to plugins.d if the package created it, otherwise zabbix_agent2.d
    REDIS_PLUGIN_CONF="${AGENT_CONF_D}/plugins.d/redis.conf"
    if [[ -f "$REDIS_PLUGIN_CONF" ]]; then
        sed -i '/^# --- Zabbix Agent Script: Redis Session Config ---$/,/^# --- End Redis Session Config ---$/d' "$REDIS_PLUGIN_CONF"
        cat >> "$REDIS_PLUGIN_CONF" <<'EOF'
# --- Zabbix Agent Script: Redis Session Config ---
Plugins.Redis.Sessions.local.Uri=tcp://localhost:6379
# --- End Redis Session Config ---
EOF
        log "  Redis plugin config written to plugins.d/redis.conf"
    else
        echo "Plugins.Redis.Sessions.local.Uri=tcp://localhost:6379" > "${AGENT_CONF_D}/redis.conf"
        log "  Redis config written to zabbix_agent2.d/redis.conf (built-in plugin)"
    fi
fi

if service_active rabbitmq-server; then
    log "  [FOUND] RabbitMQ"; DETECTED_SERVICES+=("RabbitMQ")

    # Enable the RabbitMQ management plugin (required for monitoring via HTTP API)
    if command -v rabbitmq-plugins &>/dev/null; then
        if ! rabbitmq-plugins list -e -m 2>/dev/null | grep -q '^rabbitmq_management$'; then
            log "  Enabling RabbitMQ management plugin..."
            rabbitmq-plugins enable rabbitmq_management 2>&1 | while read -r line; do log "    $line"; done
        else
            log "  RabbitMQ management plugin already enabled"
        fi
    else
        warn "  rabbitmq-plugins not found — cannot enable management plugin"
    fi

    # Create the zbx_monitor user for Zabbix monitoring
    if command -v rabbitmqctl &>/dev/null; then
        if ! rabbitmqctl list_users 2>/dev/null | grep -q '^zbx_monitor'; then
            log "  Creating RabbitMQ monitoring user zbx_monitor..."
            rabbitmqctl add_user zbx_monitor zabbix 2>&1 | while read -r line; do log "    $line"; done
            rabbitmqctl set_user_tags zbx_monitor monitoring 2>&1 | while read -r line; do log "    $line"; done
            rabbitmqctl set_permissions -p / zbx_monitor "" "" "" 2>&1 | while read -r line; do log "    $line"; done
            log "  RabbitMQ monitoring user created"
        else
            log "  RabbitMQ monitoring user zbx_monitor already exists"
        fi
    else
        warn "  rabbitmqctl not found — cannot create monitoring user"
    fi
fi

# --- Clean up stale plugin configs from older script versions ----------------
# Previous versions wrote mssql.conf directly into zabbix_agent2.d/ instead of
# plugins.d/. Remove it unconditionally — if it exists here it's always wrong.
for stale_conf in mssql.conf redis.conf; do
    if [[ -f "${AGENT_CONF_D}/${stale_conf}" ]]; then
        log "Removing stale ${AGENT_CONF_D}/${stale_conf} (now managed in plugins.d/)"
        rm -f "${AGENT_CONF_D}/${stale_conf}"
    fi
done
...
if [[ -d "${AGENT_CONF_D}/plugins.d" ]]; then
    # Build list of plugin names we need (lowercase .conf basenames)
    NEEDED_PLUGINS=()
    for svc in "${DETECTED_SERVICES[@]}"; do
        case "$svc" in
            "Microsoft SQL Server") NEEDED_PLUGINS+=("mssql") ;;
            MySQL|MariaDB)          NEEDED_PLUGINS+=("mysql") ;;
            PostgreSQL)             NEEDED_PLUGINS+=("postgresql") ;;
            Nginx)                  NEEDED_PLUGINS+=("nginx") ;;
            Apache2)                NEEDED_PLUGINS+=("apache") ;;
            Docker)                 NEEDED_PLUGINS+=("docker") ;;
            Redis)                  NEEDED_PLUGINS+=("redis") ;;
        esac
    done
    ...
    for pconf in "${AGENT_CONF_D}/plugins.d"/*.conf; do
        ...
        # Check if this plugin is in our needed list
        is_needed=false
        for needed in "${NEEDED_PLUGINS[@]}"; do
            [[ "$plugin_name" == "$needed" ]] && is_needed=true && break
        done

        if [[ "$is_needed" == "true" ]]; then
            # Ensure System.Path is uncommented for needed plugins
            sed -i 's|^#\(Plugins\..*\.System\.Path=\)|\1|' "$pconf"
            log "  Plugin enabled: ${plugin_name}"
        else
            # Comment out System.Path for unneeded plugins
            if grep -qE '^Plugins\..*\.System\.Path=' "$pconf" 2>/dev/null; then
                sed -i 's|^\(Plugins\..*\.System\.Path=\)|#\1|' "$pconf"
